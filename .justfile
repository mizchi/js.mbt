...
test-wasm:
    moon build --target wasm-gc --release modules/js/wasm
    deno run --allow-read modules/js/wasm/test_deno.ts

# Run WASM-GC DOM tests with happy-dom
test-wasm-dom:
    moon build --target wasm-gc --release modules/js/wasm
    deno run --allow-read --allow-env modules/js/wasm/test_happydom.ts
