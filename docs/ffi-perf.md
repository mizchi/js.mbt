...
| Operation | Time | Description |
|-----------|------|-------------|
| **Int to_js conversion** | 0.01 µs | Same as unsafe_cast |
| **String to_js conversion** | 0.01 µs | Same as unsafe_cast |
| **unsafe_cast Int to Js** | 0.01 µs | Baseline |
...
**Key Findings:**
- `to_js()` has same cost as `unsafe_cast` (compiler optimized)
...
- `to_js()` equals `unsafe_cast` (compiler optimized)
- Main bottlenecks:
