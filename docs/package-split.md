...
このドキュメントは `mizchi/js` を `moon.work` で複数モジュールに分割していく計画と、利用側の移行手順をまとめたものです。最初のステップとして **`mizchi/js_browser`** が独立しました。
...
├── moon.mod.json                      # mizchi/js (core + builtins + web + node + deno + bun + ...)
├── src/                               # mizchi/js のソース
└── modules/
    └── js_browser/                    # mizchi/js_browser  ← 最初の分離モジュール
        ├── moon.mod.json
        └── src/
            ├── canvas/   dom/   file/   history/
            ├── indexeddb/ location/ navigation/ navigator/
            ├── observer/ serviceworker/ storage/
            └── test_utils/            # happy-dom / jsdom helpers
```
...
ワークスペース内の `moon check` / `moon build` / `moon test` は両モジュールをまとめて処理します。

## 計画している分割

| 新モジュール                | 含めるもの                            | 状態 |
| --------------------------- | ------------------------------------- | ---- |
| `mizchi/js` (現在のルート)  | `core`, `builtins/*`, `web/*`, `mbtconv`, `examples` | 既存 |
| `mizchi/js_browser`         | `browser/*` (DOM, Canvas, ...), DOM 用 test_utils | **済** |
| `mizchi/js_node`            | `node/*` (fs, http, stream, ...)      | 未着手 |
| `mizchi/js_deno`            | `deno/*`                              | 未着手 |
| `mizchi/js_bun`             | `bun/*`                               | 未着手 |
| `mizchi/js_webextensions`   | `webextensions/*` (chrome/runtime/...) | 未着手 |
...
`moon.work` 化で build artifact が以下のように 1 階層深くなりました。スクリプトや CI が直接パスを参照している場合は更新が必要です:

| 旧パス                                                         | 新パス                                                                |
| -------------------------------------------------------------- | --------------------------------------------------------------------- |
| `target/js/release/build/deno/_tests/_tests.js`                | `target/js/release/build/mizchi/js/deno/_tests/_tests.js`             |
| `target/js/release/build/bun/bun_test/bun_test.js`             | `target/js/release/build/mizchi/js/bun/bun_test/bun_test.js`          |
| `target/wasm-gc/release/build/wasm/wasm.wasm`                  | `target/wasm-gc/release/build/mizchi/js/wasm/wasm.wasm`               |

本リポジトリ内では `deno.jsonc`, `src/wasm/test_deno.ts`, `src/wasm/test_happydom.ts` を新パスへ更新済みです。
