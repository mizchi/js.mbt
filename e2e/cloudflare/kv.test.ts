...
describe('KV Namespace', () => {
  ...
  describe('Basic Operations', () => {
    ...
    it('should return null for non-existent key', async () => {
      const value = await TEST_KV._get('non-existent');
    ...
    it('should delete a key', async () => {
      ...
      const value = await TEST_KV._get('delete-me');
      expect(value).toBeNull();
    });
  });
  ...
  describe('Data Types', () => {
    it('should store and retrieve text', async () => {
      ...
      const value = await TEST_KV._get('text-key', { type: 'text' });
    ...
    it('should store and retrieve JSON', async () => {
      ...
      const value = await TEST_KV._get('json-key', { type: 'json' });
    ...
    it('should store and retrieve arrayBuffer', async () => {
      ...
      const value = await TEST_KV._get('binary-key', { type: 'arrayBuffer' });
  ...
  describe('Expiration', () => {
    it('should set expiration with expirationTtl', async () => {
      ...
      const value = await TEST_KV._get('ttl-key');
    ...
    it('should set expiration with absolute timestamp', async () => {
      ...
      const value = await TEST_KV._get('exp-key');
      expect(value).toBe('value');
    });
  });
  ...
  describe('Cache Control', () => {
    it('should respect cacheTtl option', async () => {
      ...
      const value = await TEST_KV._get('cache-key', { cacheTtl: 300 });
  ...
  describe('Edge Cases', () => {
    it('should handle empty string value', async () => {
      ...
      const value = await TEST_KV._get('empty-key');
      expect(value).toBe('');
    });

    it('should handle unicode keys and values', async () => {
      await TEST_KV.put('emoji-🚀', 'こんにちは');
      const value = await TEST_KV._get('emoji-🚀');
    ...
    it('should handle large values', async () => {
      ...
      const value = await TEST_KV._get('large-key');
    ...
    it('should overwrite existing key', async () => {
      ...
      const value = await TEST_KV._get('overwrite-key');
