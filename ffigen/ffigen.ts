...
function toMoonbitCode(node: ts.Node, scope: string, depth = 0): string {
  ...
  if (ts.isSourceFile(node)) {
    return node.statements.map(stmt => toMoonbitCode(stmt, scope, depth + 1)).join('\n');
  }
  ...
  if (ts.isPropertySignature(node)) {
    ...
    const paramType = node.type ? toMoonbitCode(node.type, scope, depth + 1) : 'Any';
  ...
  if (ts.isClassDeclaration(node)) {
    ...
    for (const member of node.members) {
      if (ts.isConstructorDeclaration(member)) {
        ...
        methods.push(member);
      } else if (ts.isPropertyDeclaration(member) && member.type && member.name) {
    ...
    for (const member of methods) {
      ...
      const returnType = member.type ? toMoonbitCode(member.type, scope, depth + 1) : 'Val';
      const params = member.parameters.map(param => {
        const paramName = (param.name as ts.Identifier).text;
        const paramType = param.type ? toMoonbitCode(param.type, scope, depth + 1) : 'Any';
        return `${paramName}: ${paramType}`;
      }).join(', ');
      const keys = member.parameters.map(param => {
        // return 
        const paramName = (param.name as ts.Identifier).text;
        return `${paramName}`;
      }).join(', ');
    ...
    // Constructor::new
    if (constructorNode) {
      const params = constructorNode.parameters.map(param => {
        const paramName = (param.name as ts.Identifier).text;
        const paramType = param.type ? toMoonbitCode(param.type, scope, depth + 1) : 'Any';
        return `${paramName}: ${paramType}`;
      }).join(', ');
      const keys = constructorNode.parameters.map(param => {
        const paramName = (param.name as ts.Identifier).text;
        return `${paramName}`;
      }).join(', ');
    ...
    const keys = node.parameters.map(param => {
      const paramName = (param.name as ts.Identifier).text;
      return `${paramName}`;
    }).join(', ');

    const returnType = node.type ? toMoonbitCode(node.type, scope, depth + 1) : 'Unit';
      ...
      if (ts.isVariableDeclaration(decl) && decl.type && decl.name) {
        ...
        if (decl.type && ts.isTypeLiteralNode(decl.type)) {
          for (const member of decl.type.members) {
            if (ts.isMethodSignature(member) && member.name && member.name.getText() === 'new') {
