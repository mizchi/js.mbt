...
`@core` は `@js.JsImpl` trait のオーバーヘッドを削減するための軽量な JS インターロップパッケージです。

## v0.7.0 での変更

v0.7.0 以降、プロジェクト内部の実装は `@js.JsImpl` trait から `@core.Any` パターンへ移行しました。
これにより、バンドルサイズの削減とビルドパフォーマンスの向上を実現しています。

## 使い方

```moonbit
// Zero-cost type conversion
let js_val : @core.Any = @core.any(moonbit_value)
let back : MyType = js_val.cast()

// Property access
js_val["key"] = @core.any(value)
let prop = js_val["key"]

// Method call
let result = js_val._call("method", [@core.any(arg1), @core.any(arg2)])
```
