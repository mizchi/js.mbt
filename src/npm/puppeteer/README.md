...
- `env`: Custom environment variables as `@core.Any` object
...
let cookie = @js.from_map({
  "name": @core.any("session"),
  "value": @core.any("abc123"),
  "domain": @core.any(".example.com")
})
...
page.on("request", fn(req: HTTPRequest) {
  ...
  let overrides = @js.from_map({
    "headers": @js.from_map({
      "User-Agent": @core.any("Custom User Agent")
    })
  })
...
page.on("request", fn(req: HTTPRequest) {
  if req.url().contains("/api/data") {
    let response = @js.from_map({
      "status": @core.any(200),
      "contentType": @core.any("application/json"),
      "body": @core.any("{\"mock\": true}")
    })
...
let options = @js.from_map({
  "reportAnonymousScripts": @core.any(true)
})
...
let uaParams = @js.from_map({
  "userAgent": @core.any("Custom User Agent")
})
