...
export class TestDurableObject implements DurableObject {
  ...
  async alarm(): Promise<void> {
    const current = (await this.state.storage.get<number>('alarm_counter')) || 0;
...
describe('Durable Objects - Stub Communication', () => {
  ...
  it('should send POST request with body', async () => {
    ...
    const getResp = await stub.fetch('http://fake-host/storage/get?key=test-key');
  ...
  describe('Basic Operations', () => {
    ...
    it('should store different data types', async () => {
      ...
      for (const { key, value } of testCases) {
        ...
        const resp = await stub.fetch(`http://fake-host/storage/get?key=${key}`);
    ...
    it('should return null for non-existent keys', async () => {
      const resp = await stub.fetch('http://fake-host/storage/get?key=non-existent');
      const data = await resp.json<{ value: any }>();
